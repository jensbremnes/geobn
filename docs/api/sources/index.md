...
rasterio objects never leave the source modules.
...
ignore the `grid` argument and carry their own spatial metadata.

Grid-aware sources need the reference grid to know which area to query. They call `grid.extent_wgs84()` to obtain `(lon_min, lat_min, lon_max, lat_max)`
