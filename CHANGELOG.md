...
### Added
- `geobn.breakpoints.quantile(values, n)` and `geobn.breakpoints.equal_interval(values, n)` compute a breakpoint list for `set_discretization()` from data: quantile edges for bins holding roughly equal numbers of pixels, equal-width edges for bins that read as round numbers. NaN and infinite values are ignored. The outer breakpoints default to the data's min and max, and `bounds=(lo, hi)` pins them instead — either side may be `None` to keep the data's own extreme, and `quantile` accepts `±inf` for a range nothing falls outside. Data too concentrated to form `n` bins raises `ValueError` naming the repeated value and its share, rather than failing later inside `DiscretizationSpec` on a monotonicity check.
- `bn.suggest_breakpoints(node, method="quantile", n=None, bounds=None)` fetches the source registered for `node` on the configured grid and classifies its values, defaulting `n` to the node's state count in the BN so the result drops straight into `bn.set_discretization(node, bp)`. It requires a grid, as `fetch_raw()` does.
