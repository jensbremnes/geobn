...
- Evidence is no longer restricted to root nodes: `set_input(node, source)` accepts any node in the BN. An observed intermediate node can stand in for the layers behind it — a mapped terrain class instead of the slope, aspect and forest-cover rasters that feed it — and because evidence propagates in both directions, querying an ancestor of an observed node performs backward (diagnostic) inference. On the bundled Lyngen model, observing `terrain_factor="high"` moves P(`slope_angle`) from a uniform 0.25 per state to 0.006 / 0.039 / 0.372 / 0.583 over `flat, gentle, steep, extreme`. Both inference paths (per-combination queries and the single joint table), `precompute()` and its fallback loop already handled non-root evidence, including the chain-rule check for P(evidence) = 0. Note that inputs on non-root nodes are no longer independent of one another, so they can contradict the model: observing a node and its parent in a combination the CPD gives probability zero yields NaN and a `UserWarning`, as any other impossible evidence does.
- A node cannot be both an input and a query node. `infer()`, `precompute()` and `load_precomputed()` now raise `ValueError` naming the overlapping nodes, before any data is fetched. Previously this surfaced as a pgmpy error about duplicated variables, after every raster had already been downloaded and reprojected.
