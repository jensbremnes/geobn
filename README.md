...
2. **Attach sources** — each evidence node gets a `DataSource`. Any node can be an evidence node, not just the roots, so an observed intermediate node can replace the layers that feed it (and querying its parent runs the network backwards). All sources are reprojected and resampled to a common grid at inference time (the georeferenced source with the finest resolution, compared in metres, sets the grid automatically, or call `bn.set_grid()` explicitly).
